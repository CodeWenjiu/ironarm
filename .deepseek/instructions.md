...
1. **命名**：crate 命名 `robot_arm`，不含 `cu_` 前缀。binary 命名 `robot_arm`（主程序）、`robot_arm-logreader`、`robot_arm-resim`。
2. **目录**：源码在 `robot_arm/src/`，task 模块在 `src/tasks/`，消息类型在 `src/messages.rs`。新 package 在根目录下一级目录（如 `robot_arm/`）。
